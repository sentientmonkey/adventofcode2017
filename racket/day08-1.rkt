...
(define-simple-macro (compare oper env reg val)
  (oper (hash-ref env reg 0) (string->number val)))

(define (eval-cond env condition)
  (match condition
    [(list "if" reg "<" val) (compare < env reg val)]
    [(list "if" reg ">" val) (compare > env reg val)]
    [(list "if" reg ">=" val) (compare >= env reg val)]
    [(list "if" reg "<=" val) (compare <= env reg val)]
    [(list "if" reg "==" val) (compare equal? env reg val)]
    [(list "if" reg "!=" val) (not (compare equal? env reg val))]))

(define-simple-macro (operand op env reg val)
  (hash-update env reg (λ (x) (op x (string->number val))) 0))
...
(define (cpu-run str)
  (for/fold ([env (make-immutable-hash)])
      ...
      (let-values ([(oper condition) (split-at ins 3)])
...
(module+ test
  (require rackunit)
  [check-equal? (cpu-run "b inc 5 if a > 1") (make-immutable-hash)]
  [check-equal? (cpu-run "a inc 1 if b < 5") (hash "a" 1)]
  [check-equal? (cpu-run "b inc 5 if a > 1\na inc 1 if b < 5\nc dec -10 if a >= 1\nc inc -20 if c == 10")
                (hash "a" 1 "c" -10)])


(max-register (cpu-run "yxr inc 119 if nev != 6
piw dec -346 if tl != 4
